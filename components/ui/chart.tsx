  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "min-w-32 border-border/50 bg-background grid items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl",
          className
        )}
      >
        ...
        {payload.map((item) => {
          ...
          return (
            <div
              key={item.value}
              className={cn(
                "[&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground flex items-center gap-1.5"
              )}
            >
