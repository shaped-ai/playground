...
FROM text_search(
  ...
  limit=25)
ORDER BY score(expression='lightgbm_ctr')
LIMIT 50`,
        parameters: [{
          name: "query",
          type: "string" as const,
  ...
  input_user_id='105' 
      ...
      },
      {
        id: "cold_start",
        name: "Cold start",
        description: "Fetch the most highly rated films for a given user, falling back to popular",
        engine: "movielens_demo_v2",
        template: `SELECT * FROM similarity(
  embedding_ref="als_embedding" , 
  entity_type="items",
